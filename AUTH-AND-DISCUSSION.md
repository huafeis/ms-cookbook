...
默认数据保存在 `.data/reading.sqlite3`，密钥保存在 `.data/session.key`；两者均被 Git 忽略。
...
Docker 监听 7860，数据目录固定为 `/mnt/workspace/purplebook`。SQLite 适用于当前单实例部署；请备份该持久化目录。不要将多个副本分别接到本地 SQLite。
...
网站会按逻辑页面记录匿名 PV 和 UV，首页、目录、阅读路径、真实任务库、社区页和每一章分别统计。访客标识由浏览器随机生成，服务端只保存经过 HMAC 处理的哈希，不关联魔搭账号、姓名或评论数据。统计日期使用 `Asia/Shanghai`，访客去重记录保留最近 180 天。

统计页面位于 `/analytics`，打开后直接显示数据。该地址不放入读者页面的公开导航，知道固定网址即可访问。统计接口只提供聚合后的只读数据。
