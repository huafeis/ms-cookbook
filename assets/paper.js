(() => {
  ...
  $('#homePaths').innerHTML = Object.entries(paths).map(([key,p]) => `<article><a class="path-art-link" href="#paths/${key}" aria-label="${p.name}"><img src="assets/paper/${p.art}.webp" alt="" width="400" height="250"></a><h3>${p.name}</h3><p class="path-brief">${p.brief}</p><p>${p.steps}</p><a class="text-link" href="#paths/${key}">${p.link} ${arrow}</a></article>`).join('');
  ...
  function openDialog(id) {closeDialogs();$(id).showModal();document.body.classList.add('modal-open');}
  ...
  function renderChapter(chapter) {
    ...
    headingObserver=new IntersectionObserver(entries=>{
      ...
      if(visible)$$('[data-heading]').forEach(a=>a.classList.toggle('active',a.dataset.heading===visible.target.id));
    },{rootMargin:'-12% 0px -65% 0px',threshold:0});
    chapter.headings.forEach(h=>{const target=document.getElementById(h.id);if(target)headingObserver.observe(target);});
  }
  ...
  function route() {
    ...
    requestAnimationFrame(()=>{
      if(view==='reader'&&sub)document.getElementById(sub)?.scrollIntoView({block:'start',behavior:'instant'});
