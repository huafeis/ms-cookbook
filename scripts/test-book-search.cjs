...
const documents = [
  ...
  {chapterId:'chapter-21',number:21,title:'使用 DiffSynth 训练图像 LoRA',headingId:'',heading:'',text:'定制图像模型。'},
  {chapterId:'chapter-21',number:21,title:'使用 DiffSynth 训练图像 LoRA',headingId:'c21-s2',heading:'准备训练数据',text:'示例使用 metadata.csv 记录图片与提示词。'},
  {chapterId:'chapter-21',number:21,title:'使用 DiffSynth 训练图像 LoRA',headingId:'c21-s3',heading:'配置训练参数',text:'用 LoRA 调整图像风格。'},
...
test('章节标题优先于正文，标题命中不重复返回无关小节',()=>{
  ...
  assert.equal(result.results[0].chapterId,'chapter-21');
  assert.equal(result.results[0].headingId,'');
  assert(!result.results.some(r=>r.headingId==='c21-s2'));
  assert(result.results.some(r=>r.headingId==='c21-s3'));
});
test('正文匹配保留准确的小节锚点与上下文',()=>{
  const {results}=search(documents,'METADATA.CSV');
  assert.equal(results.length,1);
  assert.equal(results[0].headingId,'c21-s2');
...
test('支持章号并且空查询不倾倒整本书',()=>{
  assert.equal(search(documents,'第21章').results[0].chapterId,'chapter-21');
