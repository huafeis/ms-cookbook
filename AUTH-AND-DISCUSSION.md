...
网站会按逻辑页面记录匿名 PV 和 UV，首页、目录、阅读路径、真实任务库、社区页和每一章分别统计。访客标识由浏览器随机生成，服务端只保存经过 HMAC 处理的哈希，不关联魔搭账号、姓名或评论数据。统计日期使用 `Asia/Shanghai`，访客去重记录保留最近 180 天。统计库使用独立 SQLite 文件和回滚日志；检测到文件损坏时会保留带时间戳的故障副本并重建统计库，避免影响评论与账号数据。
