...
- 每个模型负责一个独立任务。 如OCR模型负责识别文字，版面分析模型负责判断元素位置，信息抽取模型负责从识别结果中提取需要的内容。
